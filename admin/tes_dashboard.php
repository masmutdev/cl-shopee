...
<body class="loading"
                                ...
                                <div class="card-body">
                                    <div class="d-flex">
                                        <div class="flex-grow-1">
                                            ...
                                            <h3 class="mb-0" id="totalPurchaseValue"><?php echo number_format($total_purchase_value, 2, ',', '.') ?></h3>
                                        </div>
                                ...
                                <div class="card-body">
                                    <div class="d-flex">
                                        <div class="flex-grow-1">
                                            ...
                                            <h3 class="mb-0" id="totalQtyValue"><?php echo number_format($total_qty, 0, ',', '.') ?></h3>
                                        </div>
                                        <div class="align-self-center flex-shrink-0">
                                ...
                                <div class="card-body">
                                    <div class="d-flex">
                                        <div class="flex-grow-1">
                                            ...
                                            <h3 class="mb-0" id="totalAffiliateCommisionValue"><?php echo number_format($total_affiliate_commision, 2, ',', '.') ?></h3>
                                        </div>
                                        <div class="align-self-center flex-shrink-0">
                            ...
                            <div class="card">
                                <div class="card-body p-0">
                                    ...
                                    <div class="d-flex p-3 border-bottom">
                                        <div class="flex-grow-1">
                                            <h4 class="mt-0 mb-1 fs-22" id="totalKomisiValue"><?php echo $total_komisi_sum_formatted ?></h4>
                                    ...
                                    <div class="d-flex p-3 border-bottom">
                                        <div class="flex-grow-1">
                                            <h4 class="mt-0 mb-1 fs-22" id="totalKomisiTransferBankValue"><?php echo $total_komisi_transfer_bank_formatted ?></h4>
                                    ...
                                    <div class="d-flex p-3 border-bottom">
                                        <div class="flex-grow-1">
                                            <h4 class="mt-0 mb-1 fs-22" id="totalKomisiShopeeValue"><?php echo $total_komisi_shopee_formatted ?></h4>
    ...
    <script>
        ...
        function Dashboard() {
            this.chartPendapatannya = null; // Deklarasi variabel chartPendapatannya di sini
            this.init();
        }
        ...
        Dashboard.prototype.initDateRangePicker = function() {
            ...
            $('#dash-daterange').flatpickr({
                ...
                onChange: (selectedDates) => {
                    if (selectedDates.length === 2) {
                        ...
                        // Menggunakan AJAX untuk mengambil data dari tes.php
                        $.ajax({
                            url: 'tes.php', // Ganti dengan path ke file PHP-mu
                            method: 'GET',
                            dataType: 'json',
                            data: {
                            ...
                            success: (response) => {
                                if (response) {
                                    $('#totalPurchaseValue').text(formatRupiah(response.total_purchase_value));
                                    $('#totalQtyValue').text(response.total_qty);
                                    $('#totalAffiliateCommisionValue').text(formatRupiah(response.total_affiliate_commision));
                                    $('#totalKomisiValue').text(formatRupiah(response.total_komisi_sum));
                                    $('#totalKomisiTransferBankValue').text(formatRupiah(response.total_komisi_transfer_bank));
                                    $('#totalKomisiShopeeValue').text(formatRupiah(response.total_komisi_shopee));
        ...
        $('.dropdown-item').on('click', (event) => { // Gunakan arrow function
            // Ambil periode yang dipilih dari data-periode
            var periode = $(event.currentTarget).data('periode'); // Dapatkan periode dari dropdown
            
            // Ambil tanggal saat ini
            const today = moment();
            let start_date;
            let end_date = today.format('YYYY-MM-DD'); // Tanggal hari ini

            // Tentukan start_date berdasarkan periode yang dipilih
            switch (periode) {
                case '7_hari':
                    start_date = today.subtract(7, 'days').format('YYYY-MM-DD');
                    break;
                case '15_hari':
                    start_date = today.subtract(15, 'days').format('YYYY-MM-DD');
                    break;
                case '30_hari':
                    start_date = today.subtract(30, 'days').format('YYYY-MM-DD');
                    break;
                case '6_bulan':
                    start_date = today.subtract(6, 'months').format('YYYY-MM-DD');
                    break;
                case '1_tahun':
                ...
                case 'hari_ini':
                default:
                    start_date = today.format('YYYY-MM-DD'); // Hari ini
                    break;
            }

            // Muat ulang data chart berdasarkan periode yang dipilih
            $.Dashboard.loadChartData(start_date, end_date, periode); // Memanggil loadChartData
        });

        // Fungsi untuk memuat dan memperbarui data chart
        Dashboard.prototype.loadChartData = function(startDate, endDate, periode) { // Tambahkan periode sebagai parameter
            $.ajax({
                url: 'ajax/ajax_pendapatan.php', // Ganti dengan path yang sesuai
                method: 'GET',
                dataType: 'json',
                data: {
                    start_date: startDate,
                    end_date: endDate,
                    periode: periode // Kirim periode ke server
                },
                success: (response) => {
                    if (!response || !response.total_komisi_values || !response.tanggal_values) {
                    ...
                    if (this.chartPendapatannya) {
                        this.chartPendapatannya.destroy();
                    }

                    // Render ulang chart dengan data baru
                    this.chartPendapatannya = new ApexCharts(
                        document.querySelector("#revenue-chart"),
                        options
                    );

                    this.chartPendapatannya.render();
                },
                error: (error) => {
        ...
        Dashboard.prototype.initCharts = function() {
            ...
            // Dapatkan tanggal 7 hari yang lalu dan hari ini
            const startDate = moment().subtract(7, 'days').format('YYYY-MM-DD');
            const endDate = moment().format('YYYY-MM-DD');

            // Muat data chart dengan tanggal default
            this.loadChartData(startDate, endDate);
        };

        // Inisialisasi modul aplikasi utama
        (function ($) {
            "use strict";
