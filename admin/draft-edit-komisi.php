...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $username && isset($_FILES['komisi_csv'])) {
    ...
    if (($handle = fopen($filePath, 'r')) !== FALSE) {
        ...
        while (($data = fgetcsv($handle)) !== FALSE) {
            ...
            $total_komisi = $data[2] ? $data[2] : '0.00';
            $metode_pembayaran = $data[3] ? $data[3] : 'Unknown';
            $status_pembayaran = $data[4] ? $data[4] : 'Unknown';

            // Bind parameters and execute statement
            $stmt->bindParam(':username', $username);
            $stmt->bindParam(':waktu_komisi_dibayarkan', $waktu_komisi_dibayarkan);
            $stmt->bindParam(':periode_validasi', $periode_validasi);
            ...
            $stmt->bindParam(':waktu_pembayaran', $waktu_pembayaran);
