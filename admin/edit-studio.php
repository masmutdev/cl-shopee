...
// Ambil ID studio dari parameter GET
$studio_id = isset($_GET['id']) ? intval($_GET['id']) : 0;

// Cek apakah ID studio valid
if ($studio_id > 0) {
    // Query untuk mendapatkan data studio berdasarkan ID
    $sql = "SELECT id, nama_std, alamat, host FROM studio WHERE id = :id";
    ...
    if ($stmt->rowCount() > 0) {
        ...
        $nama_std = $studio['nama_std'];
        $alamat = $studio['alamat'];
        $host = $studio['host'];
    } else {
        $errors[] = "Studio tidak ditemukan.";
    }
} else {
    $errors[] = "ID studio tidak valid.";
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Jika tidak ada error, perbarui data studio
    if (empty($errors)) {
        $sql = "UPDATE studio SET nama_std = :nama_std, alamat = :alamat, host = :host WHERE id = :id";
        $stmt = $pdo->prepare($sql);
        $stmt->bindParam(':nama_std', $nama_std, PDO::PARAM_STR);
        ...
        if ($stmt->execute()) {
            ...
            $errors[] = 'Gagal memperbarui studio: ' . implode(", ", $stmt->errorInfo());
        }
    }
}

// Tutup koneksi PDO (tidak perlu jika koneksi PDO otomatis ditutup pada akhir skrip)
$pdo = null;
?>
                        ...
                        <div class="container mt-4">
                            <div class="row">
                                <div class="col-md-8 offset-md-2">
                                    <div class="card">
                                        <div class="card-body">
                                            <h4 class="header-title mt-0 mb-1">Edit Studio</h4>
                                            ...
                                            <!-- Formulir Edit Studio -->
                                            <form action="edit-studio.php?id=<?php echo urlencode($studio_id); ?>" method="POST">
                                                <div class="mb-3">
                                                    <label class="form-label">Nama Studio</label>
                                                    <input type="text" name="nama_std" class="form-control" placeholder="Nama Studio" value="<?php echo htmlspecialchars($nama_std); ?>" required>
                                                </div>
                                                <div class="mb-3">
                                                    <label class="form-label">Alamat (Opsional)</label>
                                                    <input type="text" name="alamat" class="form-control" placeholder="Alamat Studio" value="<?php echo htmlspecialchars($alamat); ?>">
                                                </div>
                                                <div class="mb-3">
                                                    <label class="form-label">Host (Opsional)</label>
                                                    <input type="text" name="host" class="form-control" placeholder="Host Studio" value="<?php echo htmlspecialchars($host); ?>">
                                                </div>
                                                <div class="text-end">
                                                    <button type="submit" class="btn btn-primary">Perbarui Studio</button>
                                                </div>
                                            </form>
                                        </div>
                                    </div>
